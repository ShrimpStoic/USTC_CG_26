...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, src_key_padding_mask=None, triangle_pos=None):
        ...
        for i, layer in enumerate(self.layers):
            if self.encoder_skip_from_layer is not None and i == self.encoder_skip_from_layer:
                skip_x = x
            if self.encoder_skip_from_layer is not None and i == self.encoder_skip_to_layer:
                x = x + skip_x
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, ctx, src_key_padding_mask=None, triangle_pos=None, ray_pos=None, out_layers=[], tf32_mode=False, patch_h=None, patch_w=None):
        if self.rope_dim is not None:
            ...
            # Decoder RoPE Computation
            # Query RoPE (for ray patches)
            ...
            tri_freqs = self.rope_emb.get_triangle_freqs(triangle_pos)
            rope_ctx_cos, rope_ctx_sin = freqs_to_cos_sin(tri_freqs, head_dim=self.head_dim)
        else:
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, kv=ctx, src_key_padding_mask=src_key_padding_mask, rope_cos=rope_cos, rope_sin=rope_sin, rope_ctx_cos=rope_ctx_cos, rope_ctx_sin=rope_ctx_sin, patch_h=patch_h, patch_w=patch_w)
            if i in out_layers:
                out_list.append([x])
